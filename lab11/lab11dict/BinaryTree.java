...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    // Replace the following line with your solution.
	  ...
	  if(nodekey.compareTo(key) == 0){
		  ...
		  newNode = findHelper(key, node.rightChild);
	  }else if(nodekey.compareTo(key) > 0){
		  if(node.leftChild == null){
			  return null;
		  }
		  newNode = findHelper(key, node.leftChild);
	  }
	  return newNode;
  }
  ...
  public Entry remove(Object key) {
    // Replace the following line with your solution.
	  ...
	  BinaryTreeNode target = new BinaryTreeNode(null);
	  target = findHelper((Comparable) key, root);
	  if(target == root){
		  if(this.size == 1){
			  this.root = null;
			  ...
			  BinaryTreeNode cur = target.rightChild;
			  if(cur.leftChild != null){
				  while(cur.leftChild != null){
					  cur = cur.leftChild;
				  }
			  }
			  remove(cur.entry.key);
			  target.leftChild.parent = cur;
			  target.rightChild.parent = cur;
			  cur.leftChild = target.leftChild;
			  cur.rightChild = target.rightChild;
			  size ++;
			  target.leftChild = null;
			  target.rightChild = null;
			  root = cur;
	  ...
	  Comparable targetkey = (Comparable)target.entry.key;
	  Comparable parentkey = (Comparable)target.parent.entry.key;
	  if(target.leftChild == null && target.rightChild == null){
		  if(targetkey.compareTo(parentkey) > 0){
		  ...
		  BinaryTreeNode current = target.rightChild;
		  if(current.leftChild != null){
			  while(current.leftChild != null){
				  current = current.leftChild;
			  }
		  }
		  remove(current.entry.key);
		  if(target.rightChild == null){
			  if(targetkey.compareTo(parentkey) > 0){
				  target.parent.rightChild = current;
				  target.leftChild.parent = current;
				  current.parent = target.parent;
				  current.leftChild = target.leftChild;
				  target.parent = null;
				  target.leftChild = null;
				  target.rightChild = null;
			  }else{
				  target.parent.leftChild = current;
				  target.leftChild.parent = current;
				  current.parent = target.parent;
				  current.leftChild = target.leftChild;
				  target.parent = null;
			  ...
			  if(targetkey.compareTo(parentkey) > 0){
				  ...
				  current.parent = target.parent;
				  current.leftChild = target.leftChild;
				  current.rightChild = target.rightChild;
				  target.parent = null;
				  target.leftChild = null;
				  target.rightChild = null;
			  }else{
				  target.parent.leftChild = current;
				  target.leftChild.parent = current;
				  target.rightChild.parent = current;
				  current.leftChild = target.leftChild;
				  current.rightChild = target.rightChild;
				  current.parent = target.parent;
				  target.parent = null;
